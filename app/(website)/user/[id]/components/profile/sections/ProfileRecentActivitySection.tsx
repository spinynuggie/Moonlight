...
export function ProfileRecentActivitySection({
  ...
  const animateFrom = useStaggerAnimation(events.length);
  ...
  if (events.length === 0) {
    return (
      <div className="profile-crossfade-in flex min-h-[220px] items-center justify-center rounded-[14px] border border-dashed border-border/50 bg-secondary/30 text-sm text-muted-foreground">
  ...
  return (
    <div className="space-y-3">
      {events.map((event, index) => {
        ...
        return (
          <AnimatedListItem key={event.id} index={index} animateFrom={animateFrom}>
            <div
