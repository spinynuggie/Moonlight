...
function HomeLoggedIn() {
  ...
  const beatmapSearch = useBeatmapsetSearch(
    "",
    6,
  ...
  // Only animate on first ever mount AND when there's no cached data available yet.
  // Frozen at mount time to avoid re-renders as data loads in.
  const shouldAnimateEntrance = useRef(
    isFirstMount.current && !(serverStatus && newsPosts && beatmapSets.length > 0),
  ).current;
  ...
  const repeatVisitClass = !shouldAnimateEntrance && !reduceMotion
    ? "home-quick-fade-in"
    : "";
    ...
    <div className={`w-full space-y-4 ${repeatVisitClass}`}>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3 lg:gap-5">
        <div className="space-y-5 lg:col-span-2">
          <motion.div
            {...getRevealMotion(reduceMotion, shouldAnimateEntrance, 0.1)}
          ...
          <div className="grid grid-cols-1 gap-3 border-t border-border/30 pt-1 sm:grid-cols-2">
            ...
            <div
              className={newsItemAnimationClass}
              style={shouldAnimateEntrance
                ? { animationDelay: "275ms", animationFillMode: "backwards" }
                : undefined}
            >
              <SupportCard />
            </div>
        ...
        <aside className="space-y-4 lg:col-span-1">
          <motion.div
            {...getRevealMotion(reduceMotion, shouldAnimateEntrance, 0.05)}
            className={CARD_CLASS}
          >
            <CardOverlay />
            <SectionHeader
          ...
          <motion.div
            {...getRevealMotion(reduceMotion, shouldAnimateEntrance, 0.15)}
            className={CARD_CLASS}
          >
            <CardOverlay />
            <SectionHeader
